...
SampleWeight = [] #sample weight vector for each region: get from uncertainty matrix, lowest average uncertainty -> 1, highest -> 0.25 or 0.5 or 0.75
SampleWeight025 = [] #sample weight vector for each region: get from uncertainty matrix, lowest average uncertainty -> 1, highest -> 0.25 
SampleWeight05 = [] #sample weight vector for each region: get from uncertainty matrix, lowest average uncertainty -> 1, highest -> 0.5
SampleWeight075 = [] #sample weight vector for each region: get from uncertainty matrix, lowest average uncertainty -> 1, highest -> 0.75
...
def create_sample_weight():
	global SampleWeight, SampleWeight025, SampleWeight05, SampleWeight075
	avgUncertainty = range(NUM_OF_CELLS)
	for i in range(NUM_OF_CELLS):
		avgUncertainty[i] = P.T[i,:]*Uncertainty[:,i]*1.0/P.T[i,:].sum()
	SampleWeight = range(NUM_OF_CELLS)
	SampleWeight025 = range(NUM_OF_CELLS)
	SampleWeight05 = range(NUM_OF_CELLS)
	SampleWeight075 = range(NUM_OF_CELLS)
	...
	for i in range(len(avgUncertainty)):
		SampleWeight025[i] = 1 - 0.75 * (avgUncertainty[i]-minU)/(maxU-minU)
		SampleWeight05[i] = 1 - 0.5 * (avgUncertainty[i]-minU)/(maxU-minU)
		SampleWeight075[i] = 1 - 0.25 * (avgUncertainty[i]-minU)/(maxU-minU)

	SampleWeight = SampleWeight05 #default SampleWeight to 0.5 version
	...
	for count in xrange(iteration_count):
		...
		if count == 0:
			...
			if newObj < bestObj:
				...
				oldObj = bestObj
				bestObj = newObj
				if (oldObj - bestObj)*1.0/bestObj < 0.001 and count>=6:
					break
	print "count", count
...
def compare_inference_fixed_K_obfuscation(K, start_cycle, outFile, adjustmentModel):
	"""
	The whole process for a crowdsensing task considering DP-privacy and CS-reconstruction
	=========Parameters==========
	K: the sensed data received for each cycle
	...
	create_sample_weight()
	with open(outFile, "w") as outputfile:
		outputfile.write("cycle,count,mae-o,mae-w0.25,mae-w0.5,mae-w.75\n")
	n = numCycles()
	m = numCells()
	C = T[:, 0:start_cycle] #collected sensing matrix
	Zm = matrix([-100]*m).T
	...
	for cycle_i in range(start_cycle, n):
		...
		est_F = cs_matrix_recovery_sgd(C)
		est_F_lc = est_F[:,-1]
		mae_o = MAE(est_F_lc, cycle_i)
		
		global SampleWeight, SampleWeight025, SampleWeight05, SampleWeight075

		SampleWeight = SampleWeight025
		est_F = cs_matrix_recovery_sgd_weighted(C)
		est_F_lc = est_F[:,-1]
		mae_w025 = MAE(est_F_lc, cycle_i)

		SampleWeight = SampleWeight05
		est_F = cs_matrix_recovery_sgd_weighted(C)
		est_F_lc = est_F[:,-1]
		mae_w05 = MAE(est_F_lc, cycle_i)

		SampleWeight = SampleWeight075
		est_F = cs_matrix_recovery_sgd_weighted(C)
		est_F_lc = est_F[:,-1]
		mae_w075 = MAE(est_F_lc, cycle_i)		

		outputString = "{},{},{},{},{},{}\n".format(cycle_i, K, mae_o, mae_w025, mae_w05, mae_w075)
		# print outputString
		with open(outFile, "a") as outputfile:
			outputfile.write(outputString)
...
def main(argv):
	...
	for count in range(repeat):
		...
		elif obfuscation_style == "optimal-compare":
			obfuscation_file = "./obfuscation_output_GLPK/{}/{}/optimal_epsilon{}_delta{}.out".format(dataset, loss_function, e_epsilon, delta)
			read_obfuscation_matrix_cplex(obfuscation_file)
			outputfile = "./output/{}/optimal-compare-inference/{}/optimal_compare_inference_K{}_s{}_e{}_d{}_{}_{}.out".format(dataset, loss_function, K, start_cycle, e_epsilon, delta, adjustmentModelStr, current_time)
			compare_inference_fixed_K_obfuscation(K, start_cycle, outputfile, adjustmentModel)
